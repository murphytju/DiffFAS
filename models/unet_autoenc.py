...
class BeatGANsAutoencModel(BeatGANsUNetModel):
    ...
    def encoder(self,path):
        model_autoencoder = resnet18()
        model_autoencoder = torch.load(path)
    ...
    def forward_with_cond_scale(
        ...
        logits = self.forward(x,encoder, t,cond_mask = cond_mask,cond=cond, prob = 1, means_size = means_size, var_size = var_size)

        if cond_scale == 1:
            return [logits, _, _]
        cond_mask1 = prob_mask_like((x.shape[0],), prob = 0, device = x.device)
        null_logits = self.forward(x,encoder, t,cond_mask = cond_mask1, cond=cond, prob = 0, means_size = means_size, var_size = var_size)
