...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    """
    def forward(self, x, emb=None, cond=None, lateral=None, mp = None, vp = None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb=emb, cond=None, lateral=lateral)
            elif isinstance(layer, AttentionBlock):
                x = layer(x, cond, mp, vp)
            else:
                x = layer(x)
        return x
...
class AttentionBlock(nn.Module):
    """
    An optimized attention block that allows spatial positions to attend to each other.
    """
    ...
    def __init__(
        ...
        self.to_kv1 = conv_nd(1, channels, channels * 2, 1)
        self.to_q1 = conv_nd(1, channels, channels * 1, 1)
        self.to_kv2 = conv_nd(1, channels, channels * 2, 1)
        self.to_q2 = conv_nd(1, channels, channels * 1, 1)
        ...
        if use_new_attention_order:
            ...
            self.crossattention1 = QKVAttention(self.num_heads)
            self.crossattention2 = QKVAttention(self.num_heads)
        else:
            self.selfattention = QKVAttentionLegacy(self.num_heads)
            self.crossattention1 = QKVAttentionLegacy(self.num_heads)
            self.crossattention2 = QKVAttentionLegacy(self.num_heads)

        self.proj_out1 = self._init_proj_layer(zero_module(conv_nd(1, channels, channels, 1)))
        self.proj_out2 = self._init_proj_layer(zero_module(conv_nd(1, channels, channels, 1)))

    @staticmethod
    def _init_proj_layer(layer):
        init_method = 'kaiming_normal_'
        init_func = getattr(init, init_method)
        init_func(layer.weight)
        return layer

    def mean_std(self, cond, kernel_size, c):
        mean_kernel = th.ones(c, c, kernel_size, kernel_size).cuda() / (kernel_size * kernel_size)
        cond_mean = F.conv2d(cond, mean_kernel, padding=kernel_size // 2)
        squared_mean_image = F.conv2d(cond ** 2, mean_kernel, padding=kernel_size // 2)
        cond_std = squared_mean_image - cond_mean ** 2
        return cond_mean, cond_std

    def extract_mean_var(self, spatial, patch_side):
        B, C, _, side = spatial.shape
        global_mean = nn.AvgPool2d(side, side)(spatial).view(B, C, -1)
        global_var = global_mean - nn.AvgPool2d(side, side)(spatial ** 2).view(B, C, -1)
        patch_mean = nn.AvgPool2d(patch_side, patch_side)(spatial).view(B, C, -1)
        patch_var = patch_mean - nn.AvgPool2d(patch_side, patch_side)(spatial ** 2).view(B, C, -1)
        mean_stat = th.cat([global_mean, patch_mean], dim=2)
        var_stat = th.cat([global_var, patch_var], dim=2)
        return mean_stat, var_stat

    def _forward(self, x, cond, mp, vp):
        b, c, d, f = x.shape
        cond = self.bn(cond)

        
        x_mean, _ = self.extract_mean_var(x, patch_side=mp)
        _, x_std = self.extract_mean_var(x, patch_side=vp)
        cond_mean, _ = self.extract_mean_var(cond, patch_side=mp)
        _, cond_std = self.extract_mean_var(cond, patch_side=vp)

        
        h1 = self._compute_attention(x_mean, cond_mean, self.to_q1, self.to_kv1, self.crossattention1, self.proj_out1)
        h2 = self._compute_attention(x_std, cond_std, self.to_q2, self.to_kv2, self.crossattention2, self.proj_out2)

        
        h1, h2 = self._resize_heads(h1, h2, b, c, d, f)

        
        x = x.view(b, c, d, f)
        return x + self.norm(self.conv1(self.norm(x) * h2 + h1))

    def _compute_attention(self, x, cond, to_q, to_kv, crossattention, proj_out):
        q = to_q(x.view(x.size(0), x.size(1), -1))
        kv = to_kv(cond)
        qkv = th.cat([q, kv], 1)
        h = crossattention(qkv)
        return proj_out(h)

    def _resize_heads(self, h1, h2, b, c, d, f):
        h1 = self._reshape_and_interpolate(h1, b, c, d, f)
        h2 = self._reshape_and_interpolate(h2, b, c, d, f)
        return h1, h2

    @staticmethod
    def _reshape_and_interpolate(h, b, c, d, f):
        _, _, C = h.shape
        h = h[:, :, :C - 1]
        h = h.view(b, c, int(C ** 0.5), int(C ** 0.5))
        return F.interpolate(h, size=(d, f), mode='bicubic', align_corners=True)

    def forward(self, x, cond, mp, vp):
        return torch_checkpoint(self._forward, (x, cond, mp, vp), self.use_checkpoint)
